...
public class CharactersInPlay
{
    ...
    public int findMaxInFewerLines(int lines) {
        int max = counts.get(0);
        int maxIndex = 0;
        for (int i = 0; i < counts.size(); i++) {
            if (counts.get(i) < lines) {
                if (max < counts.get(i)) {
                    max = counts.get(i);
                    maxIndex = i;
                }
    ...
    public void tester() {
        ...
        int lines = 100;
        index = findMaxInFewerLines(lines);
        System.out.println("The character with the most speaking parts fewer than " + lines  
        + " is: " + names.get(index) +" " + counts.get(index));
        //for (int i = 0; i < names.size(); i++) {
            //int count = counts.get(i);
            //if (count >= MIN_PART) {               
